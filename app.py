...
# Kolom kategorikal yang punya label encoder
...
tab1, tab2 = st.tabs(["🔮 Prediksi", "📈 Visualisasi Model"])
...
with tab1:
    ...
    # Opsi dropdown kategorikal otomatis diambil dari label_encoders.classes_
    # (sudah sesuai data asli: country, city, device_type, payment_method,
    # acquisition_channel, gender, subscription_type)
    ...
    if st.button("🔍 Prediksi Churn", type="primary"):
        # Susun dataframe sesuai urutan top_features
...
with tab2:
    ...
    st.markdown(
        """
        Model yang digunakan adalah **Random Forest (Tuned)** hasil
        hyperparameter tuning menggunakan `RandomizedSearchCV`,
        dilatih pada **15 fitur terpenting** dengan data yang sudah
        melalui preprocessing (handling missing value, outlier,
        encoding, scaling) dan penyeimbangan kelas dengan **SMOTE**.
        """
    )
    ...
    metrics_df = pd.DataFrame({
        ...
        "Skor": [0.8547, 0.5141, 0.9500, 0.6672]
    })
    st.table(metrics_df)

    fig2, ax2 = plt.subplots(figsize=(6, 3))
    ax2.bar(metrics_df["Metrik"], metrics_df["Skor"], color="#3498db")
    ax2.set_ylim(0, 1)
    for i, v in enumerate(metrics_df["Skor"]):
        ax2.text(i, v + 0.02, f"{v:.3f}", ha="center")
    ax2.set_title("Performa Model — Random Forest (Tuned)")
    st.pyplot(fig2)
